...
void SoftmaxWithWeightLossLayer<Dtype>::LayerSetUp(
  ...
  LayerParameter softmax_w_param(this->layer_param_);
  softmax_w_param.set_type("Softmax");
  softmax_layer_ = LayerRegistry<Dtype>::CreateLayer(softmax_w_param);
  ...
  if (weight_by_label_freqs_) {
    ...
    for (int i = 0; i < this->layer_param_.loss_param().class_weighting_size(); i++) {
        ...
        std::cout<<label_count_data[i]<<std::endl;
    }
  }
}
...
void SoftmaxWithWeightLossLayer<Dtype>::Reshape(
  ...
  softmax_axis_ =
      bottom[0]->CanonicalAxisIndex(this->layer_param_.softmax_w_param().axis());
